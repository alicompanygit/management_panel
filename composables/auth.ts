...
export type TFetchUsersListResponse = {
  status: string;
  page: number;
  per_page: number;
  total: number;
...
export class auth implements IAuth {
  ...
  async apiAddUser(body: TAddUseBody) {
    ...
    try {
      const res = await this.fetchWithAuth<Record<string, string>>(
        `${this.config.public.baseUrl}/user_register`,
        {
          method: 'POST',
  ...
  async apiGetCurrentUser() {
    try {
      const res = await this.fetchWithAuth<TGetCurrentUser>(
        `${this.config.public.baseUrl}/get_current_user`,
        {
          method: 'GET',
        }
      );
  ...
  async apiFetchUsersList(pageNumber: number) {
    try {
      const res = await this.fetchWithAuth<TFetchUsersListResponse>(
        `${this.config.public.baseUrl}/get_users/${pageNumber}`,
        {
          method: 'GET',
        }
      );
  ...
  async apiAddSuperUser(userId: number) {
    try {
      const res = await this.fetchWithAuth<{ status: string; message: string }>(
        `${this.config.public.baseUrl}/make_super_user/${userId}`,
        {
          method: 'PUT',
        }
      );
  ...
  async apiRemoveSuperUser(userId: number) {
    try {
      const res = await this.fetchWithAuth<{ status: string; message: string }>(
        `${this.config.public.baseUrl}/remove_super_user/${userId}`,
        {
          method: 'PUT',
        }
      );
