<script setup>
import { useCustomizerStore } from '@/stores/customizer';
import { Icon } from '@iconify/vue';

const customizer = useCustomizerStore();

const props = defineProps({ item: Object, level: Number });

const handleHover = (event) => {
  customizer.parentHovered = props.item.title;
  customizer.arrow = false;

  customizer.selectedItem = [];
  customizer.activeChilds = false;
};

const handleClick = (event) => {
  customizer.parentHovered = props.item.title;
  customizer.activeParent = '';
};

const mouseleaveHandler = () => {
  //   arrow.value = false;
};
</script>

<template>
  <!---Single Item-->
  <div class="my-2">
    <v-list-item
      :to="item.type === 'external' ? '' : item.to"
      :href="item.type === 'external' ? item.to : ''"
      rounded
      :class="'d-flex gap-2 py-0 ps-3  bg-hover-' + item.BgColor"
      :color="item.BgColor"
      :ripple="false"
      :disabled="item.disabled"
      :target="item.type === 'external' ? '_blank' : ''"
      :height="37"
      @click="handleClick"
      @mouseenter="handleHover"
      @mouseleave="mouseleaveHandler"
    >
      <!-- v-scroll-to="{ el: '#top' }" -->

      <!---If icon-->
      <template v-slot:prepend>
        sdf
        <div :class="'navbox  bg-hover-' + item.BgColor" :color="item.BgColor">
          <span class="icon-box">
            <Icon
              :icon="item.icon"
              height="20"
              width="20"
              :level="level"
              :class="
                'text-white position-relative z-index-2 texthover-' +
                item.BgColor
              "
            />
          </span>
        </div>
      </template>
      <v-list-item-title
        class="mb-1 text-white text-subtitle-1 font-weight-medium"
        :color="item.BgColor"
        >{{ $t(item.title) }}</v-list-item-title
      >
      <!---If Caption-->
      <v-list-item-subtitle
        v-if="item.subCaption"
        class="text-caption mt-n1 hide-menu"
      >
        {{ item.subCaption }}
      </v-list-item-subtitle>
      <!---If any chip or label-->
      <template v-slot:append v-if="item.chip">
        <v-chip
          :color="item.chipColor"
          class="sidebarchip hide-menu"
          :size="item.chipIcon ? 'x-small' : 'x-small'"
          :variant="item.chipVariant"
          :prepend-icon="item.chipIcon"
        >
          {{ item.chip }}
        </v-chip>
      </template>
    </v-list-item>
  </div>
</template>
